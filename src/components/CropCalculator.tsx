...
function simulateRealTime(
  ...
  events.push({
    ...
    remainingMinutes: remaining * 60,
    waitFromPrevMinutes: firstWait * 60
  });
...
function TimelineCardReal({ event, index, totalEvents, currentHour, currentMinute }: { event: RealEvent; index: number; totalEvents: number; currentHour: number; currentMinute: number }) {
  ...
  switch (event.type) {
    ...
    case 'water':
      ...
      eventIcon = "💧"
      eventLabel = "浇水"
      if (event.waitFromPrevMinutes !== undefined && event.waitFromPrevMinutes > 0) {
        extraInfo = `等待 ${formatDurationShort(event.waitFromPrevMinutes / 60)} / 减少 ${formatDurationShort(event.reductionMinutes! / 60)}`
      } else {
        extraInfo = `减少 ${formatDurationShort(event.reductionMinutes! / 60)}`
      }
      break
    case 'harvest':
      dotColor = "bg-yellow-500"
  ...
  return (
    <>
      <style>{`
        ...
        .input-tooltip-wrapper .tooltip-text {
          visibility: hidden;
          opacity: 0;
        ...
        .input-tooltip-wrapper .tooltip-text::after {
          content: "";
          position: absolute;
          top: 100%;
          left: 50%;
        ...
        .input-tooltip-wrapper:hover .tooltip-text,
        .input-tooltip-wrapper input:focus + .tooltip-text {
          visibility: visible;
          opacity: 1;
        }
        /* 保持原有输入框样式，宽度由父级决定 */
        .input-tooltip-wrapper input {
          width: 100%;
        }
      `}</style>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <div className="flex gap-1">
                    <div className="input-tooltip-wrapper w-1/2">
                      <input
                        type="number"
                        value={currentHour}
                        onChange={(e) => setCurrentHour(Math.min(23, Math.max(0, Number(e.target.value))))}
                        placeholder="时"
                        className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                      />
                      <span className="tooltip-text">时</span>
                    </div>
                    ...
                    <div className="input-tooltip-wrapper w-1/2">
                      <input
                        ...
                        className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                      />
                      <span className="tooltip-text">分</span>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex gap-1">
                    <div className="input-tooltip-wrapper w-1/2">
                      <input
                        ...
                        onChange={(e) => setRemainingHours(Math.max(0, Math.min(Number(e.target.value), Math.floor(maxRemainingMinutes / 60))))}
                        placeholder="时"
                        className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                      />
                      <span className="tooltip-text">时</span>
                    ...
                    <div className="input-tooltip-wrapper w-1/2">
                      <input
                        type="number"
                        value={remainingMins}
                        onChange={(e) => setRemainingMins(Math.min(59, Math.max(0, Number(e.target.value))))}
                        placeholder="分"
                        className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                      />
                      <span className="tooltip-text">分</span>
                  ...
                  {realMode === 'nextWater' ? (
                    <div className="flex gap-1">
                      <div className="input-tooltip-wrapper w-1/2">
                        <input
                          type="number"
                          value={nextWaterHours}
                          onChange={(e) => setNextWaterHours(Math.max(0, Number(e.target.value)))}
                          placeholder="时"
                          className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                        />
                        <span className="tooltip-text">时</span>
                      ...
                      <div className="input-tooltip-wrapper w-1/2">
                        <input
                          type="number"
                          value={nextWaterMins}
                          onChange={(e) => setNextWaterMins(Math.min(59, Math.max(0, Number(e.target.value))))}
                          placeholder="分"
                          className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                        />
                        <span className="tooltip-text">分</span>
                    ...
                    <div className="flex gap-1">
                      <div className="input-tooltip-wrapper w-1/2">
                        <input
                          type="number"
                          value={reduceHours}
                          onChange={(e) => setReduceHours(Math.max(0, Number(e.target.value)))}
                          placeholder="时"
                          className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                        />
                        <span className="tooltip-text">时</span>
                      ...
                      <div className="input-tooltip-wrapper w-1/2">
                        <input
                          type="number"
                          value={reduceMins}
                          onChange={(e) => setReduceMins(Math.min(59, Math.max(0, Number(e.target.value))))}
                          placeholder="分"
                          className="rounded-xl border border-slate-700 bg-slate-950 p-2 text-center"
                        />
                        <span className="tooltip-text">分</span>
